...
import { getProductImageSource, type ProductImage, type ProductTone } from "@/lib/content";
...
export function ProductVisual({ tone, image, alt = "Foto produk Gascomp", className }: { tone: ProductTone; image?: ProductImage; alt?: string; className?: string }) {
  ...
  if (image) {
    ...
    return (
      <div className={cn("relative isolate overflow-hidden bg-[#f4f5f7]", className)}>
        ...
        {image.isPrimary && <span className="absolute bottom-3 left-3 rounded-full bg-white/90 px-2.5 py-1 text-[8px] font-extrabold tracking-[0.12em] text-[#0035b9] shadow-sm">FOTO UTAMA</span>}
      </div>
    );
  }
