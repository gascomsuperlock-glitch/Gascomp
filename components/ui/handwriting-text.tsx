...
// Self-hosted from /public. The original 21st.dev CDN copy of this font serves no
// Access-Control-Allow-Origin header, so the browser fetch() below is blocked by CORS
// and the component degrades to plain text. Same-origin sidesteps that entirely.
...
// The geometry and the measured path lengths can only be derived after the font has
// parsed and the <path> nodes exist, so this component genuinely does drive state from
// effects. The rule is off here rather than the logic being reshaped around it.
