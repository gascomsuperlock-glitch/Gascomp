...
**Save** menggunakan endpoint JSON stabil `POST /admin/content`, yang memverifikasi asal permintaan dan sesi admin sebelum membaca hingga 40 MB dan menjalankan persistensi katalog. Dengan demikian, permintaan penyimpanan browser tidak bergantung pada pengenal Server Action khusus build. Jika penyimpanan gagal, edit tetap ada di tab saat ini; pesan galat membedakan sesi kedaluwarsa, asal permintaan yang ditolak, deployment yang tidak tersedia, unggahan terlalu besar, dan batas waktu hosting. Respons yang hilang tidak membuktikan bahwa server gagal menyimpan; periksa produk sebelum mencoba lagi.

Persistensi membandingkan konten yang diterima dengan konten tersimpan dan hanya menulis produk baru atau yang berubah serta pengaturan yang berubah. Menambahkan produk baru tanpa konten bantuan memerlukan satu upsert produk, tanpa menghapus catatan anak atau menulis ulang produk lain. Pembacaan katalog yang diperlukan dan pemeriksaan skema video opsional berjalan bersamaan. Aturan penghapusan dan pengarsipan yang ada serta alur **Save**/**Cancel** eksplisit tetap berlaku. Pembersihan Storage terbatas pada produk yang diganti atau dihapus permanen; pengarsipan mempertahankan gambar dan thumbnail tutorialnya.
