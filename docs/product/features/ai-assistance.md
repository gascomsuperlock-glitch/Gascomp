...
Ayu adalah panel teks percakapan publik opsional. Asisten memperkenalkan diri sebagai Ayu; Aya adalah rekan manusia yang menerima serah terima. Asisten tidak boleh mengaku sebagai Aya maupun mengklaim Aya sudah dihubungi atau sudah membalas. Flag sisi server `GASCOMP_AI_ASSISTANCE_ENABLED=true` menggantikan tombol WhatsApp melayang; defaultnya mempertahankan tombol yang ada. Kedua kontrol disembunyikan pada rute admin. Tujuan WhatsApp garansi dan pesan klaim yang ada tidak berubah.
...
<a id="assistant-name-and-greeting-correction"></a>
## Nama asisten dan koreksi sapaan

Pemilik mengganti nama yang dilihat pelanggan dari Gascomp Assistant menjadi **Ayu**, dan menetapkan **Aya** sebagai rekan manusia yang menangani chat setelah serah terima. Sapaan, identitas, dan template serah terima bilingual, prompt sistem, teks cadangan, serta salinan antarmuka memakai kedua nama itu. Penjaga tindakan palsu sekarang juga menolak klaim bahwa Aya sudah dihubungi, dan permintaan untuk berbicara dengan Aya dikenali sebagai permintaan serah terima.

Pemilik melaporkan bahwa "hallo ka" dijawab dengan informasi pengajuan sample. Penyebabnya tiga hal. Pertama, satu entri FAQ membawa seluruh pesan pelanggan sebagai alias pencarian, termasuk sapaan pembuka "Hallo ka", sehingga sapaan polos cocok dengannya. Kedua, alias itu memuat penanda penyamaran `[ACCOUNT_ID]` yang tidak ada dalam daftar penanda yang diperiksa. Ketiga, "hallo ka" tidak dikenali sebagai sapaan sehingga tidak pernah mencapai template salam.

Sapaan polos sekarang dijawab dengan salam yang diterbitkan sebelum pencarian bukti dijalankan, jadi pesan riwayat yang kebetulan diawali sapaan tidak dapat menjawab pertanyaan yang tidak terkait. Sapaan pembuka dilucuti dari alias pertanyaan FAQ dan jawaban katalog sehingga sebuah entri hanya dapat ditemukan melalui isinya. Daftar penanda penyamaran diperluas ke seluruh penanda yang benar-benar muncul di vault, yaitu `IDENTIFIER`, `LINK`, `REDACTED`, `ORDER_NUMBER`, `ORDER_ID`, `ORDER_CONTEXT`, `ACCOUNT_ID`, `CUSTOMER`, `ADDRESS`, `PHONE`, `NAME`, dan `CREDENTIAL_REMOVED`, dan berlaku untuk pasangan transkrip maupun FAQ. Label biasa seperti `[TAMBAHAN]` pada judul listing tetap lolos karena daftarnya bernama, bukan pola umum.

Entri terbitan turun dari 874 menjadi 855 karena sembilan pasangan arsip yang membawa penanda penyamaran dan sepuluh alias sapaan kini ditolak. Seluruh 206 uji Python dan 262 uji Node lulus bersama lint, pengecekan tipe, dan pembangunan produksi, termasuk regresi baru yang memastikan sapaan polos memakai salam terbitan tanpa memanggil model dan bahwa penanda penyamaran tidak pernah menjadi alias.

<a id="symptom-only-product-inference-correction"></a>
## Koreksi penebakan produk dari gejala saja

Pemilik melaporkan bahwa asisten langsung menyebut sebuah produk padahal pelanggan belum memberi tahu produk mana yang bermasalah. Penyebabnya ada pada penyelesaian konteks produk: sebuah nama produk dianggap cocok bila satu token sepanjang minimal lima huruf hanya muncul pada satu SKU. Judul listing marketplace penuh kata kunci seperti "Anti Bocor" dan "Tidak Mudah Rusak", sehingga kata gejala dan kata negasi ikut menjadi bagian nama produk. Akibatnya "produk saya rusak" menghasilkan `BCS01-Hitam` melalui kata `rusak`, dan "tidak bisa nyala" menghasilkan SKU yang sama melalui kata `tidak`.
