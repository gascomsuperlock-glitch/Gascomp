...
- Choosing a new MP4/WebM file creates four thumbnail choices from frames inside that video before any upload begins. The administrator selects one frame; the selected WebP thumbnail and video are then uploaded together. Existing uploaded tutorials can generate a new set of frame choices without re-uploading the video.
