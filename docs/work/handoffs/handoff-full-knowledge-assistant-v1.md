...
<a id="single-duoke-root-correction"></a>
## Koreksi satu akar Duoke

Pemilik menyatakan bahwa `Douke Knowledge Base/Duoke` adalah satu-satunya sumber, dan menemukan pengetahuan terpecah di luarnya. Saat menuntaskan pemindahan vault, catatan yang dihasilkan dikembalikan ke jalur yang ditulis generatornya, yaitu `products/` dan `knowledge/approved/` di akar vault, bukan di dalam `Duoke/`. Akibatnya 55 catatan jawaban `approval: approved` dan 17 catatan katalog berada di luar `GASCOMP_AI_SOURCE_VAULT` sehingga asisten tidak pernah membacanya.

`knowledge/`, `products/`, dan `Gascomp Knowledge Base.md` dipindahkan ke dalam `Duoke/`. Struktur relatifnya dipertahankan sehingga wikilink `../../products/...` tetap terselesaikan. `DUOKE_DIR` ditambahkan pada rute Python dan `duokeDir`/`duokePath` pada rute Node; `REVIEW_DIR`, `APPROVED_DIR`, dan ekspor pengetahuan sekarang menulis di bawah `Duoke`, sementara konfigurasi Obsidian tetap di akar vault. Pembangun korpus mengindeks `knowledge/approved` sebagai jawaban dan `products` sebagai dokumen katalog bernama ber-SKU.

Sumber naik menjadi 2.444 berkas dan snapshot gabungan menjadi 874 entri sekitar 727 KiB: 35 kurasi, 55 jawaban katalog yang disetujui, 282 FAQ, 207 pasangan riwayat, dan 295 bagian produk. Seluruh 204 uji Python dan 262 uji Node lulus bersama lint dan pengecekan tipe, termasuk dua regresi baru untuk catatan jawaban dan katalog di dalam `Duoke` serta untuk catatan tanpa persetujuan pemilik.
