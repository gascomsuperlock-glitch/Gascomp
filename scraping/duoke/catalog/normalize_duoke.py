...
from scraping.shared.paths import ROOT, CAPTURE_DIR, CATALOG_PATH, REPORT_PATH, VAULT_DIR
...
def stable_slug(name: str, store_id: str | None, product_id: str) -> str:
    ...
    base = re.sub(r"[^a-z0-9]+", "-", ascii_name.lower()).strip("-")[:55] or "product"
...
def normalize_variations(
    ...
    for index, variation in enumerate(find_variation_records(record)):
        ...
        missing = [field for field, value in (("variation ID", variation_id), ("variation SKU", sku), ("variation name", name)) if not value]
        if missing:
            review.append({
                "sourceUrl": source_url,
                "productId": product_id,
                "variationIndex": index,
                "reason": f"Not imported: {', '.join(missing)} is not available as valid text.",
            })
            continue
...
def normalize_product(
    ...
    missing = [field for field, value in (("product ID", product_id), ("SKU", sku), ("product name", name)) if not value]
    if missing:
        review.append({
            "sourceUrl": source_url,
            "jsonPath": ".".join(path),
...
def main() -> int:
    ...
    if not captures:
        print(f"No product captures found in {CAPTURE_DIR}.")
        return 2
        ...
        for path, record in walk_products(capture["payload"]):
            ...
            if previous and previous != product:
                ...
                review.append({
                    ...
                    "reason": "Not imported: conflicting data was found for the same source identity.",
                })
                continue
    ...
    print(f"Valid variations: {report['importedVariations']}")
    print(f"Review items: {len(review)}")
    print(f"Admin catalog: {CATALOG_PATH}")
