...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Buka Duoke, gunakan sesi login resmi, lalu tangkap hanya respons data produk.",
    )
    parser.add_argument("--url", default=TARGET_URL, help="URL awal Duoke")
    parser.add_argument(
        "--headless",
        action="store_true",
        help="Gunakan sesi tersimpan tanpa menampilkan browser (hanya setelah login pertama).",
    )
...
def interactive_step(page: Any) -> None:
    if not page.url.startswith("https://web.duoke.com"):
        raise RuntimeError("Browser tidak berada pada domain Duoke.")

    print("\nBrowser Duoke sudah dibuka.")
    print("1. Login menggunakan akun yang berwenang bila diminta.")
    print("2. Buka halaman katalog/produk dan muat seluruh halaman produk yang akan disinkronkan.")
    print("3. Jangan membuka percakapan pelanggan selama proses ini.")
    input("Setelah katalog selesai dimuat, kembali ke terminal dan tekan Enter... ")
...
def main() -> int:
    ...
    print(f"\nSelesai: {len(saved)} respons produk disimpan; {skipped} respons lain diabaikan.")
    print(f"Indeks privat: {PRIVATE_DIR / 'capture-index.json'}")
    if not saved:
        print("Tidak ada respons produk. Pastikan halaman katalog dibuka dan datanya dimuat.")
        return 2
    return 0
