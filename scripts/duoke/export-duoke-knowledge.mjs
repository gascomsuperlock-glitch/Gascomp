...
for (const product of products) {
  ...
  const productNote = [
    ...
    `- **Model:** ${markdownText(product.model) || "Not provided"}`,
    `- **Help page:** ${baseUrl}/produk/${encodeURIComponent(product.slug)}`,
    "",
    "## Approved knowledge",
    "",
    ...(related.length ? related.map((entry) => `- [[../knowledge/approved/${entry.id}|${entry.title}]]`) : ["- No FAQs, issues, or tutorials have been approved yet."]),
    "",
  ];
...
for (const entry of entries) {
  const note = [
    ...
    "",
    `Product: [[../../products/${safeFile(entry.product.slug)}|${entry.product.name}]]`,
    "",
    "## Question",
    "",
