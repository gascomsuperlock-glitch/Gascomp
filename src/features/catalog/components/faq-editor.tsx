...
export function FaqEditor({ product, update }: { product: Product; update: (updater: (product: Product) => Product) => void }) {
  ...
  return (
    <div>
      <div className="flex items-start justify-between gap-4"><EditorHeading title="Frequently asked questions" copy="Use the language customers normally use when asking a question." /><button type="button" onClick={addFaq} className="inline-flex h-10 shrink-0 items-center gap-2 rounded-full bg-[#0035b9] px-4 text-xs font-extrabold text-white"><Plus className="size-3.5" /> Add</button></div>
        ...
        {product.faqs.map((faq, index) => (
          <article key={faq.id} className="rounded-[20px] border border-[#2c3038]/9 bg-[#fbfaf7] p-4 sm:p-5">
            <div className="flex gap-3">
              ...
              <div className="min-w-0 flex-1 space-y-4"><Field label="Question"><input value={faq.question} onChange={(event) => update((current) => ({ ...current, faqs: current.faqs.map((item) => item.id === faq.id ? { ...item, question: event.target.value } : item) }))} className={fieldClass} /></Field><Field label="Answer"><textarea value={faq.answer} onChange={(event) => update((current) => ({ ...current, faqs: current.faqs.map((item) => item.id === faq.id ? { ...item, answer: event.target.value } : item) }))} className={areaClass} /></Field></div>
              <button type="button" onClick={() => update((current) => ({ ...current, faqs: current.faqs.filter((item) => item.id !== faq.id) }))} className="grid size-8 shrink-0 place-items-center rounded-full text-[#9b7770] hover:bg-[#feeae5] hover:text-[#c9482d]" aria-label="Delete FAQ"><Trash2 className="size-3.5" /></button>
            </div>
          </article>
        ))}
