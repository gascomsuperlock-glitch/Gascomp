...
export function ProductHelpPage({ slug }: { slug: string }) {
  ...
  const copy = dictionaries[language].product;
    ...
    return (
      <div className="grid min-h-screen place-items-center bg-[#ffffff] px-5 text-center text-[#2c3038]">
        <div className="flex max-w-md flex-col items-center">
          ...
          <h1 className="mt-5 text-2xl font-extrabold">{copy.notFound}</h1>
          <p className="mt-2 text-sm text-[#6f797f]">{copy.notFoundCopy}</p>
          <Link href="/" className="mt-6 inline-flex h-11 items-center gap-2 rounded-full bg-[#2c3038] px-5 text-sm font-bold text-white"><Home className="size-4" /> {copy.returnHome}</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen w-full min-w-0 overflow-x-hidden bg-[#f8f6f0] text-[#2c3038]">
      <header className="border-b border-[#2c3038]/8 bg-white">
        <div className="mx-auto flex min-h-[68px] max-w-6xl flex-wrap items-center justify-between gap-2 px-5 py-3 sm:px-8">
          ...
          <div className="flex items-center gap-2">
            <LanguageSelector />
            <Link href="/" className="inline-flex shrink-0 items-center gap-2 text-xs font-extrabold text-[#647077] transition hover:text-[#0035b9]"><ArrowLeft className="size-4" /> <span className="hidden sm:inline">{copy.chooseAnother}</span><span className="sm:hidden">{copy.otherProduct}</span></Link>
          </div>
        </div>
      </header>
      ...
      <main>
        <section className="border-b border-[#2c3038]/8 bg-[#ffffff]">
          <div className="mx-auto max-w-6xl px-5 pb-12 pt-6 sm:px-8 sm:pb-16">
            <div className="mb-7 flex items-center gap-2 text-[11px] font-semibold text-[#899197]">
              <Link href="/" className="hover:text-[#0035b9]">{common.home}</Link>
            ...
            <div className="grid min-w-0 gap-8 rounded-[30px] border border-[#2c3038]/8 bg-white p-3 shadow-[0_16px_45px_rgba(44,48,56,0.07)] sm:p-5 md:grid-cols-[0.8fr_1.2fr] md:items-center">
              ...
              <div className="min-w-0 px-3 pb-5 md:px-6 md:pb-0">
                <span className="inline-flex items-center gap-1.5 rounded-full bg-[#eaf4ed] px-3 py-1.5 text-[10px] font-extrabold tracking-wide text-[#417a56]"><ShieldCheck className="size-3" /> {copy.officialGuide}</span>
                ...
                <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                  <a href="#tutorial" className="inline-flex h-11 items-center justify-center gap-2 rounded-full bg-[#0035b9] px-5 text-sm font-extrabold text-white"><Play className="size-4 fill-current" /> {copy.viewTutorials}</a>
                  <a href="#kendala" className="inline-flex h-11 items-center justify-center gap-2 rounded-full border border-[#2c3038]/10 px-5 text-sm font-extrabold"><LifeBuoy className="size-4 text-[#0035b9]" /> {copy.haveIssue}</a>
                </div>
              </div>
            </div>
          </div>
        ...
        <section id="tutorial" className="scroll-mt-5 px-5 py-14 sm:px-8 sm:py-18">
          <div className="mx-auto max-w-6xl">
            <SectionHeading index="01" eyebrow={copy.tutorialsEyebrow} title={copy.tutorialsTitle} />
            {activeVideo ? (
              <div className="mt-8 grid gap-5 lg:grid-cols-[1.55fr_0.75fr]">
                <div className="overflow-hidden rounded-[26px] border border-[#2c3038]/8 bg-[#2c3038] shadow-lg">
                  <TutorialPlayer url={getVideoUrl(activeVideo)} title={activeVideo.title} language={language} poster={activeVideo.thumbnailUrl} />
                ...
                <div className="space-y-2">
                  <p className="mb-3 text-[10px] font-extrabold tracking-[0.15em] text-[#899197]">{copy.videoList}</p>
                  {product.videos.map((video, index) => (
                    ...
                    >
                      {video.thumbnailUrl ? <span className="relative aspect-video w-20 shrink-0 overflow-hidden rounded-xl bg-[#2c3038]">
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={video.thumbnailUrl} alt="" className="size-full object-cover" />
                        <span className={`absolute inset-0 grid place-items-center ${video.id === activeVideo.id ? "bg-[#0035b9]/25" : "bg-black/10"}`}><Play className="size-4 fill-white text-white drop-shadow" /></span>
                      </span> : <span className={`grid size-10 shrink-0 place-items-center rounded-xl text-xs font-extrabold ${video.id === activeVideo.id ? "bg-[#0035b9] text-white" : "bg-[#f0efeb] text-[#687279]"}`}>{String(index + 1).padStart(2, "0")}</span>}
                      <span className="min-w-0"><strong className="line-clamp-2 block text-xs leading-4">{video.title}</strong><small className="mt-1 block text-[10px] text-[#8b9398]">{video.duration || common.tutorial}</small></span>
                    </button>
                  ))}
                </div>
              </div>
            ) : (
              <EmptyBlock text={copy.noVideos} />
            )}
          </div>
        </section>
        ...
        <section id="kendala" className="scroll-mt-5 bg-white px-5 py-14 sm:px-8 sm:py-18">
          <div className="mx-auto max-w-6xl">
            <SectionHeading index="02" eyebrow={copy.issuesEyebrow} title={copy.issuesTitle} />
            <p className="mt-4 max-w-xl text-sm leading-6 text-[#6d777d]">{copy.issuesCopy}</p>
                  ...
                  <article key={issue.id} className={`overflow-hidden rounded-[22px] border transition ${isOpen ? "border-[#0035b9]/30 bg-[#fffaf6]" : "border-[#2c3038]/9 bg-white"}`}>
                    ...
                    {isOpen && (
                      <div className="border-t border-[#0035b9]/12 px-5 pb-5 pt-5">
                        ...
                        <a href={getWhatsappUrl(content.whatsappNumber, product.name, issue.title, language)} target="_blank" rel="noreferrer" className="mt-5 inline-flex items-center gap-2 text-xs font-extrabold text-[#2e8250]"><MessageCircle className="size-4" /> {copy.askSupport}</a>
                      </div>
                    )}
                  </article>
                );
        ...
        <section className="px-5 py-14 sm:px-8 sm:py-18">
          <div className="mx-auto max-w-6xl">
            <SectionHeading index="03" eyebrow={copy.faqEyebrow} title={copy.faqTitle} />
        ...
        <section id="klaim-garansi" aria-labelledby="warranty-heading" className="px-5 pb-16 sm:px-8 sm:pb-20">
          <div className="mx-auto max-w-6xl rounded-[28px] border border-[#0035b9]/15 bg-white p-6 sm:p-9">
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0">
                <h2 id="warranty-heading" className="text-xl font-extrabold">{copy.warrantyTitle}</h2>
                ...
                <p className="mt-3 max-w-2xl text-sm leading-6 text-[#6a757b]">{copy.warrantyCopy}</p>
