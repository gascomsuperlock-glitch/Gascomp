  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div><h2 className="text-xl font-extrabold text-[#172b4d]">GascompCare members</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">Create buyer accounts and manage access to their virtual member cards. Accounts are saved immediately. Purchase details and claim credits are not connected yet.</p></div>
        <button type="button" disabled={busy} className={`${button} bg-[#0035b9] text-white hover:bg-[#002c98]`} onClick={() => { clearFeedback(); setSelected(undefined); setCreating(true); focusDetails(); }}><Plus aria-hidden="true" className="size-4" /> Create Member</button>
      </div>
      <div className="grid min-w-0 items-start gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(0,1fr)]">
        <section aria-label="Members" className="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 sm:p-6">
          <form onSubmit={(event) => { event.preventDefault(); clearFeedback(); setSelected(undefined); setCreating(false); setRequest((current) => ({ query: search.trim(), page: 1, revision: current.revision + 1 })); }} className="flex items-end gap-2">
            <label className="min-w-0 flex-1 text-xs font-bold text-slate-600">Search members<input type="search" value={search} maxLength={100} disabled={busy} onChange={(event) => setSearch(event.target.value)} placeholder="Name, username, or member number" className={input} /></label>
          ...
          <div className="mt-5" aria-live="polite" aria-busy={loading}>
            {loading ? <p className="flex items-center gap-2 py-10 text-sm text-slate-600"><LoaderCircle aria-hidden="true" className="size-4 animate-spin" /> Loading members...</p> : result?.error ? <div role="alert" className="rounded-xl bg-red-50 p-4 text-sm text-red-800"><p>{message(result.error)}</p><button type="button" onClick={refresh} className={`${button} mt-3`}>Retry</button></div> : !result?.members.length ? <div className="py-12 text-center"><ShieldCheck aria-hidden="true" className="mx-auto size-9 text-slate-400" /><h3 className="mt-3 font-bold">{request.query ? "No matching members" : "No members yet"}</h3><p className="mt-2 text-sm text-slate-600">{request.query ? "Try another name, username, or member number." : "Create an account after confirming the buyer's purchase."}</p></div> : <ul className="divide-y divide-slate-100">{result.members.map((member) => <li key={member.id}><button type="button" disabled={busy} aria-pressed={selected?.id === member.id} onClick={() => { clearFeedback(); setSelected(member); setCreating(false); focusDetails(); }} className={`w-full rounded-xl p-3 text-left transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#0035b9] ${selected?.id === member.id ? "bg-blue-50" : "hover:bg-slate-50"}`}><span className="block break-words text-sm font-bold text-[#172b4d]">{member.name}</span><span className="mt-1 block break-all text-xs leading-5 text-slate-600">{member.username} · {member.memberNumber}</span><span className="mt-1 block text-xs font-bold text-[#0035b9]">View member</span></button></li>)}</ul>}
          </div>
          {!loading && !result?.error && (result?.total ?? 0) > 0 && <nav aria-label="Member pages" className="mt-5 flex flex-wrap items-center gap-2 border-t border-slate-100 pt-4"><p className="mr-auto text-xs text-slate-600">{result?.total} members · Page {request.page} of {pages}</p>{Array.from({ length: Math.min(5, pages) }, (_, index) => Math.max(1, Math.min(request.page - 2, pages - 4)) + index).map((page) => <button key={page} type="button" disabled={busy || page === request.page} aria-current={page === request.page ? "page" : undefined} onClick={() => { clearFeedback(); setSelected(undefined); setCreating(false); setRequest((current) => ({ ...current, page })); }} className={`${button} px-3 ${page === request.page ? "bg-blue-50 text-blue-800" : ""}`}>{page}</button>)}</nav>}
        </section>
