...
import { isWarrantySolution, WARRANTY_TICKET_STATUSES, type WarrantySolution, type WarrantyTicketStatus } from "@/features/warranty/model/types";

export async function setWarrantyTicketStatusAction(
  ticketId: string,
  ...
  if (typeof ticketId !== "string" || !/^GWC-\d{8}-[A-F0-9]{6}$/.test(ticketId) || !WARRANTY_TICKET_STATUSES.includes(status)) {
    return { success: false, error: "The ticket data is invalid." };
  }

  try {
    const updatedAt = await updateWarrantyTicketStatus(ticketId, status, undefined);
    revalidatePath("/admin");
    return { success: true, updatedAt };
  } catch (error) {
    console.error("Warranty ticket status update failed", error);
    return { success: false, error: "The ticket status could not be updated. Check the connection and try again." };
  }
}
