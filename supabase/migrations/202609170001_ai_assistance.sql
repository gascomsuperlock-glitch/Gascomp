-- Private, transactional queue for the opt-in customer support assistant.
create table public.ai_assistance_runtime (
...
insert into public.ai_assistance_runtime(singleton) values (true);
create table public.ai_assistance_sessions (
  id uuid primary key default gen_random_uuid(),
  token_hash text not null unique,
  created_at timestamptz not null default now()
);
create table public.ai_assistance_messages (
  id uuid primary key default gen_random_uuid(),
  sequence bigint generated always as identity,
  session_id uuid not null references public.ai_assistance_sessions on delete cascade,
  role text not null check (role in ('user','assistant')),
  text text not null,
  status text not null default 'complete',
  created_at timestamptz not null default now()
);
create table public.ai_assistance_jobs (
  id uuid primary key default gen_random_uuid(),
  session_id uuid not null references public.ai_assistance_sessions on delete cascade,
  ...
  sku text,
  state text not null default 'queued' check (state in ('queued','processing','complete','handoff')),
...
create index ai_assistance_jobs_pending on public.ai_assistance_jobs(state, created_at);
create index ai_assistance_messages_session on public.ai_assistance_messages(session_id, sequence);
create table public.ai_assistance_limits (
  key text primary key,
  window_start timestamptz not null,
  count integer not null
);

alter table public.ai_assistance_runtime enable row level security;
alter table public.ai_assistance_sessions enable row level security;
alter table public.ai_assistance_messages enable row level security;
alter table public.ai_assistance_jobs enable row level security;
alter table public.ai_assistance_limits enable row level security;
revoke all on public.ai_assistance_runtime, public.ai_assistance_sessions, public.ai_assistance_messages, public.ai_assistance_jobs, public.ai_assistance_limits from public, anon, authenticated;
revoke all on sequence public.ai_assistance_messages_sequence_seq from public, anon, authenticated;

-- Runs independently of the feature flag or worker process through pg_cron.
create function public.gascomp_ai_cleanup() returns void
language plpgsql security definer set search_path = public, pg_temp as $$
begin
  perform pg_advisory_xact_lock(1786304911);
  delete from ai_assistance_sessions where created_at < now() - interval '30 days';
  delete from ai_assistance_limits where window_start < now() - interval '1 hour';
end;
$$;
revoke all on function public.gascomp_ai_cleanup() from public, anon, authenticated;
grant execute on function public.gascomp_ai_cleanup() to service_role;

create function public.gascomp_ai_command(action text, payload jsonb default '{}') returns jsonb
language plpgsql security definer set search_path = public, pg_temp as $$
declare
...
begin
  ...
  if action = 'complete' then
    ...
    if availability = 'ready' and j.expires_at > now() and j.knowledge_version = payload->>'knowledgeVersion' and j.knowledge_version = r.snapshot->>'version' then
      select e into entry from jsonb_array_elements(r.snapshot->'entries') e where e->>'id' = payload->>'answerId' and e->>'language' = j.language
        and (not (e ? 'sku') or lower(btrim(e->>'sku')) = lower(btrim(j.sku))) and e->>'kind' <> 'handoff' limit 1;
...
-- Supabase: enable pg_cron before applying this migration. Isolated PostgreSQL
-- engines without the extension can still exercise queue and cleanup behavior.
-- Named schedule is idempotent (cron.schedule replaces that owner's same name).
do $schedule$
begin
  if exists(select 1 from pg_extension where extname = 'pg_cron') then
    perform cron.schedule('gascomp-ai-retention', '*/15 * * * *', 'select public.gascomp_ai_cleanup();');
  else
    raise notice 'AI assistance retention is not scheduled. Enable pg_cron and schedule gascomp-ai-retention before public activation.';
  end if;
end;
$schedule$;
